...
模型管理总表仅展示模型名称／编码、尺寸、三个受众开关和状态；模型文件、占格／放置等信息仍可在详情页查看。总表支持逐行切换「零售／企业／内部」受众，自动保存草稿并显示保存结果，无需进入详情页。全部关闭表示对所有受众隐藏；模型总启用状态仍独立控制。统一发布时继续检查默认组合和必需配件依赖，冲突修改要求刷新，受众快改不改变尺寸、价格或已发布版本。
