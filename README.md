...
**当前状态：Shopify 嵌入式 App 已安装，模型管理首期已实现并完成真实浏览器联调。** 支持 GLB 上传、3D 预览、模块参数、草稿新建与编辑、列表搜索。已用现有 M05 模型验证上传、保存、继续编辑和刷新恢复。分类、用户可见范围、渠道价格、发布与客户配置器接口仍待开发；PVE 尚未部署。详见 [模型管理验证记录](docs/model-management-validation-20260908.md) 和 [框架安装记录](docs/scaffold-validation-20260908.md)。
...
以下是启动讨论整理出的功能范围。当前先完成模型管理与草稿，后续接分类、用户可见范围和渠道价格。

| 功能 | 预期能力 |
| --- | --- |
| 模型管理 | 已实现上传 GLB、预览、替换草稿文件；缩略图、版本历史待开发 |
| 模块参数 | 已实现尺寸、占格、放置位置、垂直偏移、正面宽度和颜色切换能力标记；承托与堆叠规则待开发 |
| 分类管理 | 维护模块分类及排序 |
| 展示与购买范围 | 分别设置零售客户、企业客户、内部员工可见与可购买的模块 |
| 渠道价格 | 管理零售价、企业价、内部价，支持币种和待报价状态 |
| 发布管理 | 已实现保存草稿与预览；发布目录、回退版本待开发 |
| 商品关联 | 关联 Shopify 商品、规格、SKU 与库存 |

按模型管理 → 分类 → 三类用户范围 → 渠道价格 → 发布与配置器接入的顺序推进，再迁移现有模块并验证渠道价格的真实结账流程。迁移前核对模型、参数和商品映射；缺失价格不自行补写。
